        ...
        >
          ...
          <el-table-column prop="forecastDemandQuantity" label="预测需求数量" width="120" align="center" show-overflow-tooltip >
            <template slot-scope="scope">
              <span :style="getForecastDemandQuantityStyle(scope.row)">{{scope.row.forecastDemandQuantity}}</span>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getDeviation(actual, predict) {
      return actual - predict
    },

    getDeviationRate(actual, predict) {
      if (predict === 0) return '-'
      const rate = ((actual - predict) / predict * 100).toFixed(1)
      return (rate > 0 ? '+' : '') + rate + '%'
    },

    getDeviationColor(actual, predict) {
    ...
    // 获取预测需求数量文本样式
    getForecastDemandQuantityStyle(row) {
