  ...
  >
    <div class="inventory-selection">
      ...
      <div class="operation-buttons">
        <el-button type="primary"  size="mini" @click="handleConfirm" :disabled="!(totalTransferQty > 0)">
          确定选择
        </el-button>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      // 存储用户输入的调拨数量
      transferQtyMap: {}, // 使用普通对象，Vue可以追踪其属性变化
      // 选中的行数据
      selectedRows: [],
      totalTransferQty:0,
  ...
  methods: {
    ...
    handleConfirm() {
      ...
      this.availableInventoryList
        .filter(item => item.transferQty > 0 && !this.selectedRows.some(row => row.uniqueCode === item.uniqueCode))
        .forEach(item => {
          selectedItems.push({
            ...item,
    ...
    loadInventoryList() {
      ...
      let param = {
        warehouseCode: this.outWarehouse.code,
