    ...
    <el-card class="search-card">
      <el-row :gutter="10">
        <el-col :xs="24" :sm="12" :md="8" :lg="6" :xl="4">
          <el-input size="mini" style="width:100%" v-model="searchForm.orderNumber" placeholder="请输入调拨订单号" clearable />
        </el-col>
        <el-col :xs="24" :sm="12" :md="8" :lg="6" :xl="4">
          <el-select size="mini" style="width:100%" v-model="searchForm.outWarehouseCode" placeholder="请选择调出仓库" clearable>
            <el-option
        ...
        <el-col :xs="24" :sm="12" :md="8" :lg="6" :xl="4">
          <el-select size="mini" style="width:100%" v-model="searchForm.inWarehouseCode" placeholder="请选择调入仓库" clearable>
            <el-option
              v-for="item in warehouses"
              :key="item.warehouseCode"
              :label="item.warehouseName"
      ...
      <el-row :gutter="10" style="margin-top:10px">
        ...
        <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="8">
          <el-row :gutter="8" class="button-group">
            ...
            <el-col :xs="12" :sm="6" :md="4" :lg="3" :xl="3" v-if="isAuth('ics:transferOrders:add')">
              <el-button size="mini" @click="handleAdd" style="width:100%">新增</el-button>
            </el-col>
            <el-col :xs="12" :sm="6" :md="4" :lg="3" :xl="3" v-if="isAuth('ics:transferOrders:issue')">
              <el-button size="mini" :disabled="selectedRow.length === 0" :loading="issueLoading" @click="handleIssue" style="width:100%">下发</el-button>
            </el-col>
            <el-col :xs="12" :sm="6" :md="4" :lg="3" :xl="4" v-if="isAuth('ics:transferOrders:withdraw')">
            ...
            <el-col :xs="12" :sm="6" :md="4" :lg="3" :xl="3" v-if="isAuth('ics:transferOrders:cancel')">
            ...
            <el-col :xs="12" :sm="6" :md="4" :lg="3" :xl="3" v-if="isAuth('ics:transferOrders:export')">
    ...
    <el-card class="table-card" style="flex: 1; display: flex; flex-direction: column;">
      <div style="flex: 1; overflow: hidden;">
        <u-table
          ...
          <u-table-column prop="inWarehouseName" label="调入仓库" width="120" align="center" show-overflow-tooltip/>
          ...
          <u-table-column label="操作" width="180" align="center" fixed="right" style="background-color: white">
            <template slot-scope="scope">
              ...
              <el-button type="text" size="small" @click="handleEdit(scope.row)" v-if="isAuth('ics:transferOrders:edit')">编辑</el-button>
            </template>
          </u-table-column>
        </u-table>
      </div>
...
export default {
  ...
  methods: {
    ...
    loadDeliveryMethodOptions() {
      API.orderConstant.getTransferDeliveryMethodList().then(({data})=>{
    ...
    handleSearch() {
      ...
      API.transferOrder.getListPage(this.searchForm).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    handleAdd() {
      ...
      this.dialogVisible = true
       this.$nextTick(() => {
        this.$refs.updateOrAddRef.addInit('新增调拨订单', this.updateOrAddDialogData);
      })
    },
    ...
    handleEdit(row) {
      ...
      this.dialogVisible = true
      this.$nextTick(() => {
        this.$refs.updateOrAddRef.updateInit('编辑调拨订单', row);
      })
    },
    ...
    handleExport() {
      ...
      API.transferOrder.exportFile(this.searchForm).then(({data}) => {
    ...
    handleIssue() {
      if (this.selectedRow.length === 0) {
        this.$message.warning('请先选择要下发的调拨订单')
        return
      }
      const orderNumbers = this.selectedRow.map(row => row.orderNumber).join('、');
      this.$confirm(`确认下发调拨订单：${orderNumbers}？`, '下发调拨订单', {
        ...
        API.transferOrder.issue(ids).then(({ data }) => {
          if (data && data.code === 0) {
            this.$message.success('调拨订单下发成功')
            this.handleSearch()
          }
    ...
    loadOrderStatusOptions(){
      API.orderConstant.getInventoryTransferOrderStatusList().then(({data})=>{
        if(data.data == data.code == 0){
    ...
    handleIssueRepeal() {
      if (this.selectedRow.length === 0) {
        this.$message.warning('请先选择要撤回的调拨订单')
        return
      }
      const orderNumbers = this.selectedRow.map(row => row.orderNumber).join('、');
      this.$confirm(`确认撤回调拨订单：${orderNumbers}？`, '撤回调拨订单', {
        ...
        const ids = this.selectedRow.map(row => row.id);
        API.transferOrder.issueRepeal(ids).then(({ data }) => {
          if (data && data.code === 0) {
            this.$message.success('调拨订单撤回成功')
            this.handleSearch()
          }
    ...
    handleCancelOrder() {
      if (this.selectedRow.length === 0) {
        this.$message.warning('请先选择要取消的调拨订单')
        return
      }
      const orderNumbers = this.selectedRow.map(row => row.orderNumber).join('、');
      this.$confirm(`确认取消调拨订单：${orderNumbers}？取消后将无法恢复。`, '取消调拨订单', {
        ...
        const ids = this.selectedRow.map(row => row.id);
        API.transferOrder.cancelOrder(ids).then(({ data }) => {
          if (data && data.code === 0) {
            this.$message.success('调拨订单取消成功')
            this.handleSearch()
          }
