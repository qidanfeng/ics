  ...
  methods: {
    init(data) {
      ...
      if (data && data.orderNumber) {
        ...
        this.id = data.id;
        this.loadCallBackRecord();
      } else {
        console.log('CallBackRecord组件初始化完成，缺少订单号');
      }
    },
    async loadCallBackRecord(){
      if (!this.orderNumber) {
      ...
      this.loading = true;
      try {
         const { data } = await API.returnSupplier.getCallbackLogByOrderNumber(this.orderNumber);
    ...
    // 重新拉取
    retryCallBack(recordId) {
      this.$confirm(`确定要重新拉取此记录吗?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        API.returnSupplier.callbackLogRetry(recordId).then(({ data }) => {
            if (data && data.code === 0) {
