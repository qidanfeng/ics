<template>
  <div class="cost-detail-container">
    <div style="margin-bottom: 4px;">
      <el-button
        v-if="isAuth('ics:returnSupplier:costDetail:reCalc')"
        type="primary"
        size="mini"
        @click="reCalcCost">
    ...
    <div class="main-content">
      ...
      <div class="left-section receivable-section">
        <div class="section-title receivable-title">
          <el-button v-if="isAuth('ics:returnSupplier:costDetail:add')" type="primary" size="mini" @click="addReceivableRow">+</el-button>
          <h3>应收费用</h3>
        </div>
          ...
          <u-table
            ...
            <u-table-column label="操作" width="160" align="center">
              <template slot-scope="scope">
                ...
                <el-button v-if="isAuth('ics:returnSupplier:costDetail:add') && scope.row.tempNewData" size="mini" type="text"  @click="saveReceivableRow(scope.row)">保存</el-button>
                <el-button size="mini" type="text" v-else @click="viewReceivableRow(scope.row)">查看</el-button>
                <el-button v-if="isAuth('ics:returnSupplier:costDetail:add') && scope.row.tempNewData" size="mini" type="text" class="delete-btn" @click="deleteReceivableRow(scope.row)">删除</el-button>
              </template>
            </u-table-column>
          </u-table>
        </div>
      ...
      <div class="right-section payable-section">
        <div class="section-title payable-title">
          <el-button v-if="isAuth('ics:returnSupplier:costDetail:add')" type="primary" size="mini" @click="addPayableRow">+</el-button>
          <h3>应付费用</h3>
        </div>
        <div class="table-container">
          <u-table
            ...
            <u-table-column label="操作" width="160" align="center">
              <template slot-scope="scope">
                ...
                <el-button v-if="isAuth('ics:returnSupplier:costDetail:add') && scope.row.tempNewData" type="text" size="mini"  @click="savePayableRow(scope.row)">保存</el-button>
                <el-button type="text" size="mini" v-else @click="viewPayableRow(scope.row)">查看</el-button>
                <el-button v-if="isAuth('ics:returnSupplier:costDetail:add') && scope.row.tempNewData" type="text" size="mini" class="delete-btn" @click="deletePayableRow(scope.row)">删除</el-button>
              </template>
            </u-table-column>
          </u-table>
        </div>
  ...
  methods: {
    ...
    loadBillData(){
      return new Promise((resolve, reject) => {
        API.returnSupplier.getBillByOrderNumber(this.orderNumber).then(({data})=>{
    ...
    reCalcCost() {
      this.$confirm(`确定重新自动计算费用吗?`, '提示', {
        ...
        API.returnSupplier.billReCalc(this.orderNumber).then(async ({data})=>{
          if (data.code === 0) {
