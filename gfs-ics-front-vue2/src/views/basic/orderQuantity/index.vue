    ...
    <el-card class="search-card">
      <el-row :gutter="10">
        ...
        <el-col :span="6" >
          ...
          <el-button size="mini" v-if="isAuth('ics:orderQuantityConfig:add')" @click="handleAdd">新增</el-button>
          <el-button size="mini" type="danger" v-if="isAuth('ics:orderQuantityConfig:delete')" :disabled="selectedRow.length === 0" :loading="deleteLoading" @click="handleDelete">删除</el-button>
          <el-button size="mini" v-if="isAuth('ics:orderQuantityConfig:export')" :loading="exportLoading" @click="handleExport">导出</el-button>
          <el-button size="mini" v-if="isAuth('ics:orderQuantityConfig:import')" :loading="importLoading" @click="handleImport()">导入</el-button>
        </el-col>
      </el-row>
      ...
      <div style="flex: 1; overflow: hidden;">
          ...
          <u-table-column label="操作" width="100" align="center" fixed="right">
            <template slot-scope="scope">
              <el-button type="text" size="small" v-if="isAuth('ics:orderQuantityConfig:edit')" @click="handleEdit(scope.row)">编辑</el-button>
            </template>
          </u-table-column>
        </u-table>
        </div>
...
export default {
  ...
  methods: {
    ...
    handleSearch() {
      ...
      API.orderQuantityConfig.getListPage(this.searchForm).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    handleDelete() {
      if (this.selectedRow.length === 0) {
        this.$message.warning('请先选择要删除的订购量配置')
        return
      }
      this.deleteLoading = true
      this.$confirm('确认删除该订购量配置吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        const ids = this.selectedRow.map(item => item.id)
       API.orderQuantityConfig.deleteByIds(ids).then(({ data }) => {
         if (data && data.code === 0) {
    ...
    handleImport() {
      ...
      let templateCode = "1003";
      let importApiUrl = API.orderQuantityConfig.importFile();
    ...
    handleExport() {
      ...
      API.orderQuantityConfig.exportFile(this.searchForm).then(({data}) => {
