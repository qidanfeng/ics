<template>
  <div class="app-container">
    ...
    <el-card class="search-card">
      <el-row :gutter="10">
        <el-col :span="4">
          <el-select size="mini" style="width:100%" v-model="searchForm.costType" placeholder="请选择费用类型" clearable>
            <el-option
        ...
        <el-col :span="4">
          <el-select size="mini" style="width:100%" v-model="searchForm.documentTypeCode" placeholder="请选择单据类型" clearable>
            <el-option
        ...
        <el-col :span="6">
          ...
          <el-button size="mini" v-if="isAuth('ics:costCalculationConfig:add')" @click="handleAdd">新增</el-button>
        </el-col>
      </el-row>
    </el-card>
      ...
      <div style="flex: 1; overflow: hidden;">
          ...
          <u-table-column label="操作" width="100" align="center" fixed="right">
            <template slot-scope="scope">
              <el-button type="text" size="small" v-if="isAuth('ics:costCalculationConfig:edit')" @click="handleEdit(scope.row)">编辑</el-button>
            </template>
          </u-table-column>
        </u-table>
        </div>
    ...
    <!-- 新增/编辑子组件 -->
    <update-or-add v-if="showUpdateOrAdd" ref="updateOrAddRef" :document-type-options="documentTypeOptions" :cost-type-options="costTypeOptions" @submit="handleFormSubmit" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    loadCostTypeList(){
      API.orderConstant.getCostTypeList().then(({data})=>{
        if(data.data && data.code ==0 ){
          this.costTypeOptions = data.data;
        }
      }).catch(error=>{
    ...
    handleSearch() {
      ...
      API.billCalculationConfig.getListPage(this.searchForm).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    handleDelete() {
      this.$confirm('确认删除选中的费用计算配置吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        const ids = this.selectedRow.map(item => item.id)
        API.billCalculationConfig.deleteByIds(ids).then(({ data }) => {
          if (data && data.code === 0) {
