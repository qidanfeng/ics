<template>
  <div>
    <el-table
      ...
      <el-table-column prop="inStockQuantity" label="入库数量" width="100" align="center"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    async loadOrderDetailData(orderNumber) {
      try {
        // 调用API获取订单明细数据
        const { data } = await API.inOrder.getDetailListByOrderNumber(orderNumber);
