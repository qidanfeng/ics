      ...
      <el-table-column label="操作" width="160" align="center">
        <template slot-scope="scope">
          <div v-if="!scope.row.lineNumber">
            <el-button v-if="isAuth('ics:inOrders:costDetailInfo:edit') && !scope.row.editing" size="mini" type="text" @click="editRow(scope.row)">修改</el-button>
            <el-button v-if="isAuth('ics:inOrders:costDetailInfo:edit') && scope.row.editing" size="mini" type="text" @click="saveRow(scope.row)">保存</el-button>
            <el-button v-if="isAuth('ics:inOrders:costDetailInfo:delete')" size="mini" type="text" class="delete-btn" @click="deleteRow(scope.row)">删除</el-button>
          </div>
        </template>
      </el-table-column>
    </el-table>
  ...
  methods: {
    ...
    deleteRow(row) {
      this.$confirm(`确定要删除第[${row.index}]行吗?`, '提示', {
        ...
        API.inOrder.billDelete([row.id]).then(({data})=>{
          if (data.code === 0) {
            this.changedData = true;
