...
defineOptions({ name: 'AlgorithmDescription' });

withDefaults(
  defineProps<{
    algorithmType?: string;
    selectedForecastMethod?: string;
    forecastResults?: Record<string, any>;
  }>(),
  {
    algorithmType: 'sma',
    selectedForecastMethod: 'sma',
    forecastResults: () => ({})
  }
);
...
function formatForecasts(forecasts: any) {
  if (!forecasts || !Array.isArray(forecasts)) return '';
  return forecasts.map((f: number) => Math.round(f)).join(' → ');
}
</script>
