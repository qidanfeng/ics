            ...
            <div class="operation-detail">
              <div class="status-section">
                <div class="status-tag">
                  ...
                  >
                    <el-icon style="margin-right: 4px;"><component :is="getTimelineIconComp(record.resStatus)" /></el-icon>
...
import { User, Clock, Refresh, CircleCheck, CircleClose, InfoFilled } from '@element-plus/icons-vue';
import type { Component } from 'vue';
import { getOrderIssueLogByOrderNumber, issue as issueApi } from '@/service/api/inventory/return-supplier';

defineOptions({ name: 'ReturnSupplierIssueRecord' });

const recordData = ref<any[]>([]);
const loading = ref(false);
const orderNumber = ref('');
const id = ref<any>(null);
const issueStatus = ref<any>(null);

/** 初始化 */
function init(data: any) {
  if (data && data.orderNumber) {
    orderNumber.value = data.orderNumber;
    issueStatus.value = data.issueStatus;
    id.value = data.id;
    loadIssueRecord();
  } else {
...
/** 加载下发记录 */
async function loadIssueRecord() {
  ...
  loading.value = true;
  try {
    const { response } = await getOrderIssueLogByOrderNumber(orderNumber.value);
...
/** 重新下发 */
function retryIssue() {
  ElMessageBox.confirm(`确定要重新下发此订单吗?`, '提示', {
...
/** 获取时间轴图标组件 */
function getTimelineIconComp(status: string): Component {
