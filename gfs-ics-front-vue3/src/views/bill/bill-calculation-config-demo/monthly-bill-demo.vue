      ...
      >
        ...
        <el-table-column prop="dailyBillCount" label="日结单数量" width="110" align="center">
          <template #default="{ row }">
            <el-link type="primary" @click="showDetail(row)">{{ row.dailyBillCount }}</el-link>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="totalAmount" label="总金额(元)" width="130" align="right">
          <template #default="{ row }">
            <span style="color: #E6A23C; font-weight: bold; font-size: 14px">{{ row.totalAmount.toFixed(2) }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="250" align="center" fixed="right">
          <template #default="{ row }">
            ...
            <el-button type="text" size="small" @click="handleInvoice(row)" v-if="row.status === 'CONFIRMED'">开票</el-button>
    ...
    >
      ...
      >
        ...
        <el-table-column label="操作" width="120" align="center">
          <template #default="{ row }">
            <el-button type="text" size="small" @click="viewDailyBill(row)">查看日结单</el-button>
          </template>
        </el-table-column>
      </el-table>
...
defineOptions({ name: 'MonthlyBill' });
...
const dailyBillDetailData = ref<any[]>([]);
...
function showDetail(row: any) {
  ...
  // 模拟日结账单明细数据
  dailyBillDetailData.value = [];
  for (let i = 1; i <= row.dailyBillCount; i++) {
    const day = i.toString().padStart(2, '0');
    dailyBillDetailData.value.push({
      id: `${row.id}-${i}`,
  ...
  // 计算税额
  dailyBillDetailData.value.forEach(item => {
...
function viewDailyBill(row: any) {
  ElMessage.info(`跳转到日结账单：${row.dailyBillNo}（演示）`);
...
function handleConfirm(row: any) {
  ElMessageBox.confirm('确认该月结账单吗？', '提示', {
...
function handleInvoice(row: any) {
  ElMessageBox.confirm('确认该月结账单已开票吗？', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(() => {
