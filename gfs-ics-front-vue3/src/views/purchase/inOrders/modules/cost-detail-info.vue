      ...
      <el-table-column label="操作" width="160" align="center">
        <template #default="scope">
          <div v-if="!scope.row.lineNumber">
            <el-button v-if="hasAuth('ics:inOrders:costDetailInfo:edit') && !scope.row.editing" size="small" type="text" @click="editRow(scope.row)">修改</el-button>
            <el-button v-if="hasAuth('ics:inOrders:costDetailInfo:edit') && scope.row.editing" size="small" type="text" @click="saveRow(scope.row)">保存</el-button>
            <el-button v-if="hasAuth('ics:inOrders:costDetailInfo:delete')" size="small" type="text" class="delete-btn" @click="deleteRow(scope.row)">删除</el-button>
          </div>
        </template>
      </el-table-column>
    </el-table>
